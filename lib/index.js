    ...
    onProxyReq: (proxyReq, req, res) => {
      ...
      const doSaveReqParams = data => {
        triggerSaveReqParams = () => {
          ...
          const {
            type: oldType
          } = getFileContent(reqparamsTypeFilePath);
          const typeContent = json2Interface(data, `${interfacePrefixName}ReqparamsI`);
          const canUpdate = data && !oldType || (differ || insideDiffer)({
            data,
            oldData: null,
            typeContent,
      ...
      modifyResponse(res, proxyRes, function (body) {
        ...
        return triggerSaveReqParams().then(() => {
          const {
            ...
            schema,
            type: oldType
          } = getFileContent(resbodyTypeFilePath, resbodyJsonFilePath, schemaFilePath);
          apiLog({
            url,
            method,
          ...
          const canUpdate = body && !oldType || (differ || insideDiffer)({
            data: body,
            oldData: json,
            typeContent,
            oldTypeContent: oldType,
